...
class QuidditchPlayersApiDataSourceTest : BaseTest() {
    ...
    @Test
    fun `houses happy success flow`() = runTest {
    ...
    @Test
    fun `get players by house success flow`( ) = runTest {
    ...
    @Test
    fun`fetch players and positions success flow`() = runTest {
    ...
    @Test
    fun `fetch status by house name success flow`() = runTest {

        // when
        Mockito.`when`(mockQuidditchPlayersApi.getStatusByHouseName(houseName = HouseName.RAVENCLAW.name)).thenReturn(MockData.mockStatusDeferredResponseSuccess)
    ...
    @Test
    fun `fetch status by player id success flow`() = runTest {

        // given
        val playerId = MockData.ravenclawTeam().first().id.toString()

        // when
        Mockito.`when`(mockQuidditchPlayersApi.fetchStatusByPlayerId(playerId = playerId)).thenReturn(MockData.mockStatusDeferredResponseSuccess)
